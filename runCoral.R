...
synth <- function(substrateOne, substrateTwo, max) {
  ...
  #if (is.nan(substrateOne)) {
 #   return(1)
    
 # } else if (is.nan(substrateTwo)) {
#    return(2)
    
#  } else if (substrateOne*substrateTwo*max==0) {
 #   return(0)
...
environment <- function(time, L, N, X) {
  ...
  # Light
  outL <- if (L[3]==0) {
    seq(L[1], L[2], along.with = time)
    
  } else if (L[3]==1) {
    seq(L[2], L[1], along.with = time)
  ...
  # DIN
  outN <- if (N[3]==0) {
    seq(N[1], N[2], along.with = time)
    
  } else if (N[3]==1) {
    seq(N[2], N[1], along.with = time)
  ...
  # Prey
  outX <- if (X[3]==0) {
    seq(X[1], X[2], along.with = time)
    
  } else if (X[3]==1) {
    seq(X[2], X[1], along.with = time)
...
runCoral <- function(time, env, pars_HX, pars_S) {
  ...
  for(var in c("rho_N", "j_eC", "j_CO2", "j_HG","r_CH", "dH.Hdt", "H", "HS")){
    assign(var, rep(NA, times=length(time)))
  }
  #Create variable lists for Symbiont
  for (var in c("j_L", "j_CP", "j_eL", "j_NPQ", "j_SG", "rho_C", "j_ST", "r_CS", "c_ROS", "dS.Sdt", "S", "SH", "S.t")) {
    assign(var, rep(NA, times=length(time)))
  }
  
  #Initial Host fluxes
  j_X <- rep(x = MMk(substrate = env$X[1], max = pars_HX$j_Xm, halfSat = pars_HX$K_X), times = length(time)) #Uptake of prey
  j_N <- rep(x = MMk(substrate = env$N[1], max = pars_HX$j_Nm, halfSat = pars_HX$K_N), times = length(time))
  ...
  j_CP[1] <- alwaysPositive(rejFlux=(synth(substrateOne = j_L[1] * pars_S$y_CL, substrateTwo = j_CO2[1]*H[1]/pars_S$S_0, max = pars_S$j_CPm))) #Initial production of biomass from photosynthesis
  ...
  for (t in (2:length(time))) {
    S.t[t] <- sum(S[t-1])  #For use when having more than one symbiont. For now, use:
    ...
    j_CP[t] <- synth(substrateOne = (j_L[t] * pars_S$y_CL), substrateTwo = ((j_CO2[t-1] + r_CH[t-1])*H[t-1]/S.t[t] + r_CS[t]), max = (pars_S$j_CPm)) / c_ROS[t-1] #New photosynthesis rate
    ...
    j_SG[t] <- synth(substrateOne = (pars_S$y_C*j_CP[t]), substrateTwo = ((rho_N[t-1]*H[t-1]/S.t[t] + r_NS[t])/pars_S$n_NS), max = (pars_S$j_SGm)) #Biomass production of Symbiont
    ...
    rho_N[t] <- max((j_N[t] + pars_HX$n_NX * j_X[t] + r_NH[t] - pars_HX$n_NH * j_HG[t]), 0) #Nitrogen shared with Symbiont
    j_eC[t] <- max((j_X[t] + rho_C.t/H[t-1] - j_HG[t]/pars_S$y_C), 0) #Excess carbon in the system
  ...
  return(list(time=time, L=env$L, N=env$N, j_X=j_X, j_N=j_N, r_NH=r_NH, rho_N=rho_N, j_eC=j_eC, j_CO2=j_CO2, 
       j_HG=j_HG, r_CH=r_CH, dH.Hdt=dH.Hdt, H=H, r_NS=r_NS, j_L=j_L, 
       j_CP=j_CP, j_eL=j_eL, j_NPQ=j_NPQ, j_SG=j_SG, rho_C=rho_C, j_ST=j_ST, 
       r_CS=r_CS, c_ROS=c_ROS, dS.Sdt=dS.Sdt, S=S, HS=HS, SH=SH, S.t=S.t))
}
